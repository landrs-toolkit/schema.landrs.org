...
function getConfigFromEnvVars () {
  ...
  const DEFAULT_PROJECT_TITLE = 'LANDRS Ontology Manager'
  const editor = {
    ...
    meta: {
      title: _.get(editorConfig, 'head.title', DEFAULT_PROJECT_TITLE),
      customerName: process.env.CUSTOMER_NAME || 'University of Notre Dame and Zazuko GmbH',
      description: _.get(editorConfig, 'head.description', 'Linked Data Ontology Manager for Domain Specialists')
    },
    logoUrl: '/landrs-logo-full.svg',
    text: {
      groupName: 'Container',
      home: [
    ...
    committer: {
      ...
      email: _.get(editorConfig, 'committer.email', 'landrs@nd.edu')
    }
  }
